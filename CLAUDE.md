...
| **MCP interno = lectura + `create_task`** | `src/lib/mcp/` + `POST /api/mcp` (ruta `src/app/api/[transport]/route.ts`, mcp-handler, SSE deshabilitado). Auth: Bearer `MCP_API_KEY` (tiempo constante; **sin la env var rechaza todo**). Los tools usan el **admin client** (service role). **Fase 2 (ADR-015):** primera y única escritura = `create_task` (crea un GitHub Issue; reporter fijo `"Asistente (MCP)"`). Todo lo demás es lectura. NUNCA agregar más tools de escritura sin decisión explícita del usuario. Reutilizan `business-rules.ts`/`github.ts`; respuestas ya resueltas (descripción DYMMSA, totales, ubicación oculta sin stock). Ver ADR-015. |
...
| 6 — Mejoras UX | 🔄 | — |
| 7 — MCP interno | 🔄 lectura ✅ · escritura `create_task` ✅ · más escrituras por definir | `src/lib/mcp/` + `/api/mcp` |
