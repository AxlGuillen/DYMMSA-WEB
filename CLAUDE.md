...
| **Ingresos desde Odoo (#94)** | Se **LEEN**, jamás se copian a una tabla: `account.payment` inbound (`in_process`/`paid`) por `date` dentro del mes = mismo hecho que `paid_at` en egresos. Lo facturado y no cobrado va aparte (por cobrar / vencido, `OPEN_RECEIVABLES_DOMAIN`). Dos lecturas por mes cacheadas con `unstable_cache` (solo en `income-cache.ts`; `revalidateTag(tag, { expire: 0 })` para purgar — el segundo argumento es obligatorio). **Si Odoo falta o falla, la ruta responde 200 con `income: null`** y el overview degrada a solo egresos; nunca 500. Ver ADR-027. |
