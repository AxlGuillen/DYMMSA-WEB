...
- `src/lib/tours/<modulo>.ts` — pasos por módulo (`OverviewStep[]`). **14 tours** (cobertura completa desde la issue #74, 2026-08-21): `dashboard.ts` (sidebar + panel de inicio), `cut-planner.ts` (compartido por el planner de orden y el corte rápido — mismo TourButton en ambos modos), `approval.ts` (página pública; tono sin jerga interna), `purchase-planner.ts`, `quoter.ts` (cubre sus DOS momentos: upload y editor — los pasos del momento ausente se filtran solos), `quotations-list.ts`, `quotation-detail.ts` (link de aprobación y ✓/✗ son condicionales por estado), `orders-list.ts`, `order-detail.ts`, `inventory.ts`, `etm-db.ts`, `urrea-catalog.ts`, `suppliers.ts` y `tasks.ts` — y desde la issue #120 (2026-09-25) **18**: `finance-overview.ts` (mes, egresos, ingresos de Odoo, cierre, notas de crédito, vencimientos por semana), `payables.ts`, `hours.ts` (el selector de persona es admin-only y se filtra solo para un member) y `team.ts` (para un member solo queda el encabezado: la tabla vive detrás de `AdminOnly`). Fuera a propósito: `/docs`, `/changelog`, `/orders/new`, `/materials` y `/hours/import` (autoexplicativas; el panel de importación ya explica el formato). Los pasos de footers fijos llevan `side: 'top'`; los de headers, `side: 'bottom'`.
- **Anti-drift del sidebar (#120)**: `tour` es prop obligatoria de `NavSection` — una sección nueva sin ancla no compila — y `tours.test.tsx` exige que cada `[data-tour^="nav-"]` renderizado sea un paso de `DASHBOARD_TOUR`. Antes Finanzas y Horas existían en el menú sin ancla y el tour de Inicio las saltaba en silencio. Límite conocido: los tests protegen **selectores**, no texto; la auditoría del #120 encontró tres pasos que decían menos de lo que había en pantalla (`nav-dymmsa`, `sup-table`, `plan-actions`). Al tocar una pantalla con tour, releer sus pasos.
