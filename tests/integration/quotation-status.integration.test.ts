/**
 * Quotation status transitions against local Supabase (ADR-021): token
 * regeneration, approved_at sealing/preservation and the reopen guard.
 */
...
describe('PATCH /quotations/[id]/status (integración local)', () => {
  test('cambiar estado regenera approval_token y preserva is_approved de los ítems', async () => {
    ...
    expect(q.approval_token).not.toBe(token) // regenerated token: the old link dies

    const [item] = await sql<{ is_approved: boolean | null }>(
      'SELECT is_approved FROM quotation_items WHERE id = $1', [itemIds[0]],
    )
    expect(item.is_approved).toBe(true)
  })
  ...
  test('approved sella approved_at y lo PRESERVA al re-aprobar (no lo pisa)', async () => {
    ...
    // Back to draft and re-approve: approved_at must keep the ORIGINAL date.
    ...
    // pg returns a Date, so compare by value.
