/**
 * CRUDs against local Supabase (ADR-021): covers what the mock cannot —
 * real constraints (composite UNIQUE, normalization, FK without cascade) and RLS.
 */
...
describe('urrea_catalog CRUD (constraints reales)', () => {
  test('crea normalizando code/brand a trim+upper y persiste así en la BD', async () => {
    ...
    expect(row).toMatchObject({ code: 'CRUD-1', brand: 'URREA' })
  })
  ...
  test('duplicado (mismo code+brand, otra capitalización) → 400 por el UNIQUE real', async () => {
    ...
    // Different casing normalizes into the same (code, brand).
...
describe('brands CRUD (FK real bloquea borrar en uso)', () => {
  test('marca asignada a un proveedor NO se puede borrar (400); libre sí (200)', async () => {
    ...
    // Assign it to a supplier (state setup via raw SQL).
    await sql(
    ...
    expect(await sql('SELECT 1 FROM brands WHERE id = $1', [brand.id])).toHaveLength(1)
