...
- `src/lib/tours/<modulo>.ts` — pasos por módulo (`OverviewStep[]`). Tours actuales: **`dashboard.ts`** (sidebar sección por sección + panel de inicio — lo que ve alguien al entrar), **`cut-planner.ts`** (planificador de corte, 8 bloques), **`approval.ts`** (página pública del cliente; tono sin jerga interna) y **`purchase-planner.ts`** (planificador de compra, 8 bloques: resumen económico → umbrales → vistas → grupos y su decisión → compra local → Excel/guardar; los pasos del footer fijo llevan `side: 'top'`).
...
- Queda para después (si se necesita): auto-arranque en primera visita por módulo (localStorage) y tours para el cotizador e inventario. Por ahora la decisión explícita es que sean **opcionales, no obligatorios**.
