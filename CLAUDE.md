...
| **Bloque Odoo del MCP (ADR-025)** | La facturación OFICIAL vive en Odoo Online (tercero, plan Custom) — el MCP la LEE, jamás escribe ni la cruza con DYMMSA-WEB. `src/lib/odoo/` aislado (patrón github.ts): cliente **JSON-2** (`/json/2/…` — `/jsonrpc` muere en Online 21.1), **cola serializada** (1 req en vuelo, espaciado 1.1 s, backoff 429: el rate limit de Odoo Online no admite paralelas) y **`catalog.ts` como frontera**: modelos/campos allowlisted también en filtros; nómina/salarios NUNCA entran. Tools con prefijo `odoo_*` en `src/lib/mcp/tools/odoo/`, respuestas digeridas (read_group server-side, many2one→nombre, false→null). Env opcional (`ODOO_URL`/`ODOO_API_KEY`/`ODOO_DB`): sin ella el MCP opera y el health reporta `skip`. Bloque fases 1–6 (issues #65/#70): contabilidad, contactos+ventas, inventario Odoo (vía `stock.quant` — `qty_available` es computado no-almacenado, fuera del catálogo), empleados (sin nómina), flotilla, detalle de documentos con líneas + timbrado CFDI (`l10n_mx_edi_*` en la whitelist), y complementos de pago REP (`l10n_mx_edi.document` — la verdad del timbrado de pagos; los `l10n_mx_edi_*` de `account.payment` computan `false` y NO entran) — y **fase 7 (#110): el vínculo REAL factura↔orden de venta** (`odoo_invoice_link_check`: huérfanas y vínculos rotos del periodo en una llamada, paginando por dentro; `invoice_origin` es texto y NO prueba el vínculo) — 14 tools `odoo_*`. Concepto `readOnlyFields` en el catálogo: campos legibles bajo demanda pero vedados en dominios/order (`reconciled_invoice_ids` devuelve 0 en silencio; `sale_order_count` e `invoice_ids` truenan — ambos computados sin store). `narration` es HTML → `htmlToText()`. |
...
| 6 — Mejoras UX | 🔄 | — |
| 7 — MCP interno | 🔄 lectura ✅ · escrituras acotadas (`create_task`, `update_task`, `set_inventory_location`, `mark_payable_paid`, `create_payable`) ✅ · OAuth 2.1 Supabase (conectores) ✅ · bloque Odoo (14 tools, fases 1-7) ✅ · horas (3 tools de lectura, #101) ✅ · cobertura de todos los módulos + test anti-drift (44 tools, #109/#110) ✅ · núcleo transaccional solo-lectura por decisión | `src/lib/mcp/` + `/api/mcp` + `src/lib/odoo/` |
