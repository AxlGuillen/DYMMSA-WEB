...
| **Excel URREA** | Se genera desde las **decisiones de mayoreo guardadas** (`order_purchase_decisions`): piezas = `packages_wholesale × std_snapshot` (múltiplos de STD). Criterio = **pertenencia a `urrea_catalog`** por `catalogKey(model_code, brand)` — cualquier línea del catálogo (URREA/SURTEK/FOY...), **ya NO `brand='URREA'`**. **Se descarga desde el planificador** (issue #54), junto con el Excel de compra local — ya NO desde `OrderDetail`, que solo conserva el Formato de Entrega. Como el archivo debe reflejar lo guardado, si hay cambios en pantalla el botón **persiste primero y luego genera** (`isDirty` compara la elección efectiva contra la decisión guardada, incluidas las stale); con grupos sin decidir ("Revisar") no genera y avisa. Separadores y `quantity_to_order=0` siguen fuera (nunca llegan al plan). |
...
| **Módulo de corte (#59)** | Piezas DYMMSA (tubos/placas de cobre) que se MANDAN A HACER. Unidades SIEMPRE en mm; los `numeric` de supabase-js llegan como string → se coercen en la frontera del API, nunca en la lib. Necesidad neta = `Σ (longitud + margen) × cantidad` (sobreestima a propósito: es cifra para pedir). Margen `cut_margin_mm` en settings (default 20, **0 válido**), se cobra POR PARTICIÓN; la última puede caer a ras. El Excel URREA/compra local NO incluyen esto: el material de corte se pide con su propio Excel desde `/dashboard/orders/[id]/cutting`. Presentaciones capturadas → `material_presentations` (catálogo que se arma solo; solo tubos). Helpers en `src/lib/cut-plan.ts` (NO en business-rules). Ver ADR-022. |
