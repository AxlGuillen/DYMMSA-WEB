...
| `GET` | `/api/time-entries` | ✅ | Query: `user (uuid, solo admin — un member lo IGNORA y recibe lo propio)`, `from`, `to` (ISO; default semana actual lunes→domingo). Devuelve `{ user, from, to, entries, week }` con `week = buildWeekView()` (7 días, totales derivados). **`week` solo es válido cuando `from..to` es exactamente una semana lunes→domingo** (la UI siempre manda eso; con otro rango, `week` arranca en el lunes de `from` y su total no es el de la semana). `time` normalizado a `HH:MM` |
