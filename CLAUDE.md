...
# Bloque Odoo del MCP (ADR-025) — lectura de la facturación (opcional: sin esto las tools odoo_* avisan)
ODOO_URL=       # solo el origen, ej. https://<empresa>.odoo.com
ODOO_API_KEY=   # API key con expiración (rotar 6-12 meses); la de exploración se revoca al desplegar
ODOO_DB=        # opcional (Odoo Online: una sola base)
...
**`store_inventory`** — `model_code TEXT UNIQUE`, `quantity INTEGER CHECK >= 0`, `location TEXT` (ubicación física/gaveta, texto libre opcional; **se conserva aunque `quantity=0`** — metadato duradero, no transaccional; solo se oculta en el frontend sin stock). **Marca:** no vive aquí — se resuelve por `model_code` contra `etm_products` en la vista **`store_inventory_with_brand`** (issue #53), que prefiere la fila que SÍ trae marca (muchos ETM la tienen vacía) y deja `NULL` si ninguna la tiene ("sin marca", categoría filtrable). `GET /api/inventory` lee de la vista para filtrar antes de paginar; el selector usa la RPC `inventory_brand_counts()`. Import Excel acepta columna `ubicacion` (alias `ubicación`/`location`/`gaveta`, case-insensitive); en modo `upsert` **no** pisa la existente si el archivo no la trae.
...
**`cut_plan_pieces` / `material_presentations`** — módulo de corte (issue #59, Fase 1): piezas de tubo/placa DYMMSA que se mandan a hacer. `cut_plan_pieces` por orden (FK CASCADE; CHECK `cut_piece_shape`: tubo → `diameter_mm`, placa → `thickness_mm`+`width_mm`; `requested_label` registra el match cliente↔medida; `source_item_id` → order_items SET NULL). `material_presentations` = catálogo del proveedor que **se arma solo** al capturar ("barras de 6 m"); UNIQUE **NULLS NOT DISTINCT** contra duplicados. Unidades SIEMPRE en **mm** (numeric). `etm_products` ganó 5 columnas nominales `cut_*` que SOLO pre-llenan la lista (la verdad del plan es `cut_plan_pieces`).

**`suppliers` / `brands` / `supplier_brands`** — módulo de proveedores de menudeo (issue #21, standalone). `suppliers`: contacto (`name UNIQUE`, phone, whatsapp, email, address, notes) + `payment_terms_days` (INTEGER nullable, issue #84 — días de crédito; NULL = contado; pre-llena el vencimiento de las facturas por pagar). Se captura con un Select de presets `PAYMENT_TERM_PRESETS` (7/15/30/60/90 → 1 semana…3 meses) más "Otro…" con input libre (#92): un valor guardado fuera de la lista abre en Otro con su número, y `paymentTermsLabel()` lo pinta en tablas y selects. `brands`: catálogo global de marcas, `name UNIQUE` **normalizado trim+upper** (`normalizeBrandTag()` — sin default URREA) para cruce futuro **por valor** con marcas de productos; sembrada con las existentes. `supplier_brands`: M2M — `supplier_id` CASCADE, `brand_id` **sin cascade** (eliminar marca en uso se bloquea con 400 + conteo). POST de supplier con links hace **rollback** del padre si fallan; PATCH de marcas es **replace por diff**.

**`payables`** — módulo Finanzas fase 1 (issue #84): registro simbólico de gastos por pagar — **la facturación oficial vive en Odoo** (Odoo solo factura a clientes; aquí van los gastos propios). `supplier_id` FK **sin cascade** (borrar proveedor con facturas → 400), `concept`, `amount CHECK > 0`, `invoice_date`, `due_date` (pre-llenada en UI con `invoice_date + payment_terms_days` del proveedor, editable), status CHECK:
```
pending | paid | cancelled
```
`paid_at` = fecha **REAL** de pago (puede diferir del vencimiento — es el dato de la planeación fiscal). Regla del PATCH: `status→'paid'` sin `paid_at` → default hoy; `status→'pending'` lo limpia; el POST ignora status del cliente (siempre nace `pending`). Desde #100 el popup de editar trae **Estado + "Pagada el"** (habilitada solo con *Pagada*; el form manda `status`/`paid_at` **solo si cambiaron**, porque un `status: 'paid'` suelto re-sella hoy) y la lista filtra por **proveedor (uuid) y rango de monto** (`supplier`, `minAmount`, `maxAmount`; `min > max` → 400). **Quién la marcó pagada vive en `audit_events`, nunca en esta tabla** (ver abajo y ADR-028). Matemática del overview en `src/lib/payables.ts` (NO en business-rules; reloj por parámetro). **El "hoy" del negocio se toma con `todayInMexico()`** (`format.ts`, zona `America/Mexico_City`), no con `formatISODate()`: el server corre en UTC y de las 18:00 a la medianoche local sellaría el pago del día siguiente. Fronteras de mes con `nextMonth()` — `${month}-31` no existe en meses cortos y Postgres responde 22008: vencidas cuentan aunque vengan de meses previos, pagadas por mes de `paid_at`, canceladas fuera de todo. Las columnas `date` se pintan con `useDateFormat()` → `formatDay(iso, formato)` (`DATE_FORMATS` en `format.ts`; nunca `new Date()` sobre la fecha cruda). **Fase 2 (#94, ADR-027): los ingresos se LEEN de Odoo** — `GET /api/finance/income?month=` (cobros `account.payment` inbound por fecha real de pago + facturas abiertas partidas en por cobrar/vencido; Data Cache 15 min con tag `finance-income` y `POST …/refresh`) — y el overview pinta el **cierre del mes** (`monthClosing()` en `src/lib/income.ts`: real = cobrado − pagado, proyectado = real − pendientes del mes − vencidas de meses previos, `carryOverTotal` de `summarizeMonth`). Pendiente: simulación de mover pagos entre meses.

**`profiles` / `time_entries` / `time_imports`** — módulo de horas (issue #93, ADR-026). `profiles` 1:1 con `auth.users` (trigger `handle_new_user` SECURITY DEFINER: si falla, **bloquea el alta** a propósito): `display_name`, `role` CHECK:
```
admin | member
```
y `clock_employee_id` (UNIQUE; NULL = no checa; es el número entre paréntesis del reporte NGTeco), y **`shift`** (`full_time | part_time | NULL`, #101, ADR-029): la jornada de la persona, referencia de las gráficas de Horas — `SHIFT_HOURS` en `src/lib/timesheet.ts` fija 8 h/4 h al día y **40 h/20 h a la semana** (5 días, decisión 2026-09-24); NULL = sin jornada (las dos líneas se dibujan sin resaltar ninguna). Se edita en Equipo. `time_entries` = **una fila por pareja de checada**: `work_date`, `source_clock_in` (**inmutable**, lo que dijo el checador; llave `UNIQUE(user_id, work_date, source_clock_in)` de idempotencia del import), `clock_in`, `clock_out` (NULL = olvidó checar salida), `note`, `source` CHECK `import | manual`, rastro `edited_by/edited_at/original` (`original` se escribe **solo la primera vez**; **solo un cambio de hora sella el rastro** — una nota sola no congela la fila para el import). FK `user_id → profiles` **sin cascade**. Totales **no se persisten** (`buildWeekView()` en `src/lib/timesheet.ts`, NO en business-rules). Las gráficas (#101) tampoco calculan nada: `weekChartData`/`shiftProgress`/`buildWeeklyTrend` preparan cada número y `WeekBars`/`TrendBars` (recharts vía `ui/chart.tsx`, cargadas con `next/dynamic`) solo pintan; una checada sin salida se pinta aparte porque no suma. `time_imports` = bitácora por carga. RPC `import_time_entries` (INVOKER, transaccional): upsert `WHERE edited_at IS NULL` → las filas corregidas por un admin sobreviven al re-import y se reportan como `skipped_edited`. **Primera RLS por persona** del esquema: SELECT `user_id = auth.uid() OR is_admin()`, escrituras `is_admin()`; `profiles`: cada quien lee su fila y el admin todas, solo el admin edita; `time_imports` solo la lee el admin; GRANTs **sin `anon`**. **La policy debe decir lo mismo que la ruta**: el MCP lee con el token del usuario, así que una policy más abierta que `requireAdmin` es una puerta lateral (review PR #99).

**`audit_events`** — bitácora genérica (issue #100, ADR-028): `entity_type` (`'payable'` hoy), `entity_id`, `action` (`created | status_changed | paid_at_changed | deleted`), `actor_id` (uuid **sin FK**: un INSERT de bitácora jamás tumba la escritura del usuario — review PR #106), `actor_name` (**snapshot** del `display_name` de ese día), `data` jsonb (`{ from, to }` o el snapshot de la fila), `created_at`. **Solo el admin la lee** (RLS `is_admin()`); `authenticated` **no tiene INSERT**: escribe únicamente el trigger `payables_audit` → `audit_payable()` (SECURITY DEFINER, `search_path=''`) al cambiar `status`/`paid_at`, crear o borrar. Con `service_role` el actor es `NULL` ("Sistema"). Sumar otra entidad = otro trigger sobre la misma tabla. GRANTs sin `anon`.
...
| **Excel URREA** | Se genera desde las **decisiones de mayoreo guardadas** (`order_purchase_decisions`): piezas = `packages_wholesale × std_snapshot` (múltiplos de STD). Criterio = **pertenencia a `urrea_catalog`** por `catalogKey(model_code, brand)` — cualquier línea del catálogo (URREA/SURTEK/FOY...), **ya NO `brand='URREA'`**. **Se descarga desde el planificador** (issue #54), junto con el Excel de compra local — ya NO desde `OrderDetail`, que solo conserva el Formato de Entrega. Botón **"Copiar para Excel"** (issue #64): copia `código⇥cantidad` tab-separado para pegar en el Excel viejo de URREA (su sistema no abre bien el .xlsm moderno). Como archivo y copiado deben reflejar lo guardado, si hay cambios en pantalla el botón **persiste primero y luego genera** (`isDirty` compara la elección efectiva contra la decisión guardada, incluidas las stale); con grupos sin decidir ("Revisar") no genera y avisa. Separadores y `quantity_to_order=0` siguen fuera (nunca llegan al plan). |
...
| **Módulo de corte (#59)** | Piezas DYMMSA (tubos/placas de cobre) que se MANDAN A HACER. Unidades SIEMPRE en mm; los `numeric` de supabase-js llegan como string → se coercen en la frontera del API, nunca en la lib. Necesidad neta = `Σ (longitud + margen) × cantidad` (sobreestima a propósito: es cifra para pedir). Margen `cut_margin_mm` en settings (default 20, **0 válido**), se cobra POR PARTICIÓN; la última puede caer a ras. El Excel URREA/compra local NO incluyen esto: el material de corte se pide con su propio Excel desde `/dashboard/orders/[id]/cutting`. La placa se compra como **HOJA de medida fija** (ancho × largo del proveedor; `packSheets`: filas + paginado en hojas — issue #64, antes era tira por largo). Presentaciones capturadas → `material_presentations` (catálogo que se arma solo; barras de tubo Y hojas de placa); se administran en `/dashboard/materials` (alta manual + borrar erróneas, issue #71). **Corte rápido** (`/dashboard/cutting`, sidebar): `CutPlanner` en modo `standalone` — EFÍMERO por diseño (piezas en el store `dymmsa-cut-draft` de localStorage, jamás en BD; el guardado de lista está apagado, solo registra presentaciones); se siembra desde `QuotationDetail` vía `GET /api/quotations/[id]/cut-candidates` (separadores e `is_sold=false` fuera). Helpers en `src/lib/cut-plan.ts` (NO en business-rules). Ver ADR-022. |
...
| **MCP remoto = OAuth 2.1 de Supabase (ADR-023)** | `src/lib/mcp/` + `POST /api/mcp` (ruta `src/app/api/[transport]/route.ts`, mcp-handler + `withMcpAuth`, SSE deshabilitado). Auth: **OAuth nativo de Supabase** — `verifyToken` = `getUser` contra GoTrue + claim `client_id` en allowlist (`MCP_OAUTH_CLIENT_IDS`); un token de sesión web NO abre el conector. **CERO service_role**: cada llamada construye su cliente con el token del request (`clientForToken`/`contextFrom`) → RLS aplica como en la app. El proxy NO intercepta `/api/mcp` ni `/.well-known/*`; consentimiento en `/oauth/consent` (detrás del login, conserva `?next=`). Conector: Claude web/móvil/Code vía OAuth. **Escrituras aprobadas como dirección (decisión 2026-07-12, ADR-015):** hoy son 5 — `create_task`, `update_task` (comentar/priorizar/cerrar; NUNCA edita título/descripción), `set_inventory_location` (solo la gaveta, jamás cantidades) — issue #72 — y, primer módulo fuera del núcleo (#109, ADR-030), `mark_payable_paid` (pagada con fecha real o de regreso a pendiente) y `create_payable` (nace `pending`; vencimiento = fecha + plazo del proveedor). Editar/borrar facturas se queda en la app. Regla de pago compartida con el `PATCH` en `resolvePaymentUpdate()`; la bitácora atribuye la escritura por MCP sola (trigger con `auth.uid()`). El núcleo transaccional (inventario-cantidades, cotizaciones, órdenes) queda solo-lectura por decisión explícita (2026-08-20); se diseña con el usuario antes de implementar. **Cobertura de lectura = toda ruta `GET`** (#109): `tests/mcp/coverage.test.ts` exige que cada ruta esté cubierta por una tool o excluida con motivo — un módulo nuevo sin tool rompe el CI. Las búsquedas por nombre (proveedor, orden, factura, persona) pasan por `requireSingleMatch`: varias coincidencias = error que las lista, nunca adivinar. La agrupación app↔Odoo vive en `SERVER_INSTRUCTIONS` (mapa de bloques + reglas; el listado MCP es plano). Reutilizan `business-rules.ts`/`github.ts`/helpers de `src/lib`; respuestas ya resueltas (descripción DYMMSA, totales, ubicación oculta sin stock). |
| **Bloque Odoo del MCP (ADR-025)** | La facturación OFICIAL vive en Odoo Online (tercero, plan Custom) — el MCP la LEE, jamás escribe ni la cruza con DYMMSA-WEB. `src/lib/odoo/` aislado (patrón github.ts): cliente **JSON-2** (`/json/2/…` — `/jsonrpc` muere en Online 21.1), **cola serializada** (1 req en vuelo, espaciado 1.1 s, backoff 429: el rate limit de Odoo Online no admite paralelas) y **`catalog.ts` como frontera**: modelos/campos allowlisted también en filtros; nómina/salarios NUNCA entran. Tools con prefijo `odoo_*` en `src/lib/mcp/tools/odoo/`, respuestas digeridas (read_group server-side, many2one→nombre, false→null). Env opcional (`ODOO_URL`/`ODOO_API_KEY`/`ODOO_DB`): sin ella el MCP opera y el health reporta `skip`. Bloque fases 1–6 (issues #65/#70): contabilidad, contactos+ventas, inventario Odoo (vía `stock.quant` — `qty_available` es computado no-almacenado, fuera del catálogo), empleados (sin nómina), flotilla, detalle de documentos con líneas + timbrado CFDI (`l10n_mx_edi_*` en la whitelist), y complementos de pago REP (`l10n_mx_edi.document` — la verdad del timbrado de pagos; los `l10n_mx_edi_*` de `account.payment` computan `false` y NO entran) — **fase 7 (#110): el vínculo REAL factura↔orden de venta** (`odoo_invoice_link_check`: huérfanas y vínculos rotos del periodo en una llamada, paginando por dentro; `invoice_origin` es texto y NO prueba el vínculo) — y **fase 8 (#113): cartera por cliente** (`total_due`/`total_overdue`/`days_sales_outstanding` de `res.partner` como `readOnlyFields`; `odoo_receivables_ranking` ordena en memoria porque Odoo no ordena por computados; `cartera` es saldo contable NETO de notas de crédito — no se cuadra contra la facturación bruta, #102) — 15 tools `odoo_*`. Concepto `readOnlyFields` en el catálogo: campos legibles bajo demanda pero vedados en dominios/order (`reconciled_invoice_ids` devuelve 0 en silencio; `sale_order_count` e `invoice_ids` truenan — ambos computados sin store). `narration` es HTML → `htmlToText()`. |
| **Ingresos desde Odoo (#94)** | Se **LEEN**, jamás se copian a una tabla: `account.payment` inbound **de cliente** (`partner_type = customer`; los reembolsos de proveedor también son inbound) en `in_process`/`paid` por `date` dentro del mes = mismo hecho que `paid_at` en egresos. Lo facturado y no cobrado va aparte (por cobrar / vencido, `OPEN_RECEIVABLES_DOMAIN`). **Notas de crédito (#102):** las `out_refund` abiertas viajan en la MISMA lectura (`OPEN_CUSTOMER_MOVES_DOMAIN`, `move_type in`) y se muestran aparte como saldo a favor del cliente — **jamás se restan** del por cobrar ni del vencido (decisión 2026-09-21: no se sabe si el cliente la usará); `splitReceivables` las salta y `summarizeCreditNotes` las suma. Las tools `odoo_overdue_invoices`/`odoo_invoices_summary`/`odoo_customer_profile` las reportan en `notas_credito*` con la misma regla. Dos lecturas por mes cacheadas con `unstable_cache` (solo en `income-cache.ts`; `revalidateTag(tag, { expire: 0 })` para purgar — el segundo argumento es obligatorio). **Si Odoo falta o falla, la ruta responde 200 con `income: null`** y el overview degrada a solo egresos; nunca 500. Ver ADR-027. |
| **Bitácora admin-only (#100)** | Lo que un member **no debe saber que se guarda** no puede vivir en una tabla que lee: cualquier `authenticated` lee `payables` directo por PostgREST con su token. Por eso el rastro va en `audit_events` con RLS `is_admin()` y lo escribe un **trigger DEFINER**, no la ruta. `GET /api/payables` anexa `paid_by` **solo si el llamador es admin y solo en filas pagadas hoy** (una regresada a pendiente conserva su evento viejo) — para un member la respuesta **no trae la llave** (ni `null`); `GET /api/payables/[id]/events` es `requireAdmin`; la columna "Pagada por" y el historial del popup solo existen con `useProfile().isAdmin` (`payablesColumns(isAdmin)`). La única prueba real es `tests/integration/payables-audit.integration.test.ts`. Ver ADR-028. |
| **Horas por MCP (#101)** | `get_week_hours`, `get_hours_trend`, `list_time_imports` (`src/lib/mcp/tools/hours.ts`) son solo lectura y **no traen lógica de permisos**: `run()` pasa el contexto y "yo" es `ctx.userId`; `persona` busca en `profiles` y la RLS por persona decide (a un member le devuelve solo su fila → 0 coincidencias con mensaje claro; el admin ve a todos). `list_time_imports` a un member da 0 filas y lo dice. Nada de escrituras. Ver ADR-029. |
| **Permisos por rol (#93)** | `is_admin()` + RLS en la BD, **`requireAdmin()`** en el handler (403 descriptivo) y `useProfile().isAdmin` en el cliente — las tres, nunca solo ocultando en el frontend. El `GET /api/time-entries` de un member **ignora `?user=`** y fuerza su propio id. Las correcciones de un admin nunca tocan `source_clock_in`. Borrar un usuario de `auth.users` con checadas falla con 23503 a propósito: la baja es desactivar, no borrar. Ver ADR-026. |
...
- **Todo en inglés:** código, variables, nombres de BD, API routes y **comentarios**. El texto que ve el usuario (labels, mensajes de error de la API, toasts) y los nombres de los tests siguen en español.
- **TypeScript estricto.** Types centralizados en `src/types/database.ts`.
- **Hooks = TanStack Query + fetch a API Routes propias.** No llamar Supabase directo desde el cliente — ni lecturas ni CRUD. Wrapper compartido `fetchJson`/`ApiError` en `src/lib/fetch-json.ts`. Excepción legítima: `useAuth`/`login` usan el browser client para la sesión. **Migrados (2026-06-19):** cotizaciones (incl. lista/stats/detalle), inventario, catálogo URREA. **Migrado (2026-07-28, issue #55):** `useProducts` → `GET/POST /api/products` + `PATCH/DELETE /api/products/[id]`. **Pendientes (aún con lecturas directas):** `useOrders`, `useDashboard` → migrar a su `GET /api/*` cuando se toquen.
...
- **Zustand store:** `dymmsa-quotation-draft` en localStorage. Llamar `reset()` al guardar exitosamente. Otras keys persistidas: `dymmsa-columns` (columnas ocultas por tabla, issue #18 — vía `useVisibleColumns` + `ColumnPicker`), `dymmsa-column-widths` (anchos arrastrados por tabla, issue #55 — vía `useColumnWidths` + `ResizableHead`; solo guarda lo que el usuario ajustó, el resto sale del `width` declarado en la columna), `dymmsa-sidebar-collapsed`, `dymmsa-sound`, `dymmsa-discrete-mode`, `dymmsa-date-format` (formato de las columnas `date` de Finanzas y Horas, issue #92 — vía `useDateFormat` + `DateFormatPicker`; default largo, y un valor desconocido cae al default).
- **Vistas guiadas:** driver.js con propósito de **overview** (explican los bloques de la pantalla, no paso-a-paso). Config compartida en `src/lib/tours/index.ts` (`startOverview`, español, filtra pasos ausentes); pasos por módulo en `src/lib/tours/<modulo>.ts` anclados a atributos **`data-tour="..."`** (nunca clases). Cada página con tour: botón "Vista guiada" (`CircleHelp`) + **test anti-drift** que verifica que todos los selectores existen con el fixture completo. Tema en `globals.css` (`.driver-popover.dymmsa-tour`). Ver ADR-024. Tours actuales: **18** — dashboard+sidebar, corte (compartido orden/rápido), aprobación pública, planificador de compra, cotizador (2 momentos), listas y detalles de cotizaciones y órdenes, inventario, BD ETM, catálogo URREA, proveedores, tareas (issue #74), Finanzas overview, Facturas por pagar, Mi semana y Equipo (issue #120; fuera a propósito: docs/changelog/orders-new/materials/hours-import). **`tour` es prop obligatoria de `NavSection`** en el sidebar: una sección nueva sin ancla no compila, y `tours.test.tsx` exige que todo `nav-*` tenga su paso en el tour de Inicio — los tests protegen selectores, no texto: al cambiar una pantalla, releer su tour (#120 encontró tres pasos desfasados). Botón compartido: `<TourButton tour="…" />` (`src/components/tours/`). Siempre **opcionales** — nunca auto-arrancan.
- **Gráficas:** recharts a través de `src/components/ui/chart.tsx` (Chart de shadcn; tema en `--chart-1..5`). Siempre en dos capas: la card con encabezado/estados y el SVG en su propio archivo cargado con `next/dynamic` + `ssr: false` (`WeekBars`, `TrendBars`, `OrderStatusDonut`) — parte el bundle y deja que los tests afirmen sobre el encabezado (jsdom no mide contenedores). La matemática va en `src/lib/`, nunca en el componente. `shadcn add chart` reescribe `card.tsx` para importar de un paquete `cn` — revertir; el helper es `@/lib/utils`.
...
- **Documentación in-app** (`/dashboard/docs`): una página con índice de anclas; las secciones nuevas viven en `src/app/dashboard/docs/sections/*.tsx` sobre el marco `DocSection`. La sección **Asistente (IA)** se renderiza de `src/lib/mcp/manifest.ts` (nombre, bloque, módulo, lectura/escritura, ejemplo, límites) y `tests/mcp/manifest.test.ts` truena si `server.ts` registra una tool que no está en el manifiesto — **agregar una tool = agregar su fila** (ADR-031). Al cambiar una regla que la doc describe (recepción, Excel URREA, planificador…), actualizar su sección en el mismo PR.
- **Comentarios: en inglés, cortos y solo el WHY.** Máximo 2 líneas —idealmente 1— con referencia corta (`ADR-XXX` / `#NN`). Si no cabe en 2 líneas casi siempre es que explica el QUÉ, que el código ya dice: la narrativa vive en CLAUDE.md y los ADRs, nunca en el código. Se comenta la trampa, el invariante o el porqué de un workaround; nada más. Las directivas (`eslint-disable`, `@ts-expect-error`) son funcionales: conservan su explicación, acortada.
...
- **Comando:** `bun run test` (1127 tests). Watch: `bun run test:watch`. Coverage: `bun run test:coverage`.
...
- **Seed** (`supabase/seed.sql`): dos usuarios de prueba (`test@dymmsa.local` admin, `member@dymmsa.local` member; password `testpassword123`) + fixtures deterministas. Nunca datos reales.
- **Integración**: reusa `injectSupabaseServer`/`injectSupabaseAdmin` pero devuelve un cliente **real autenticado** (no mock). `resetDb()` (pg) aísla por test (nunca trunca `profiles`). Env/llaves demo en `.env.test.example`. Dos usuarios sembrados: `test@dymmsa.local` (admin) y `member@dymmsa.local` (member, checador 5) — `authedClientAs(LOCAL.member)` para probar RLS por persona (#93). **Obligatorio antes de mergear a `main` cualquier cambio de RLS/roles**: el mock no la finge.
...
**Modelo:** Claude Opus 5 con `--effort high` (fijado en `claude_args` del workflow, via OAuth — sin costo adicional de API). Opciones de effort: `low`/`medium`/`high`/`xhigh`/`max`.
...
| 6 — Mejoras UX | 🔄 | — |
| 7 — MCP interno | 🔄 lectura ✅ · escrituras acotadas (`create_task`, `update_task`, `set_inventory_location`, `mark_payable_paid`, `create_payable`) ✅ · OAuth 2.1 Supabase (conectores) ✅ · bloque Odoo (15 tools, fases 1-8) ✅ · horas (3 tools de lectura, #101) ✅ · cobertura de todos los módulos + test anti-drift (44 tools, #109/#110/#113) ✅ · núcleo transaccional solo-lectura por decisión | `src/lib/mcp/` + `/api/mcp` + `src/lib/odoo/` |
...
| **Decisión técnica no obvia** | Crear `DYMMSA/04-Decisiones-Tecnicas/ADR-XXX-nombre.md` (último: ADR-031) |
