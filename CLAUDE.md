...
| **Bitácora admin-only (#100)** | Lo que un member **no debe saber que se guarda** no puede vivir en una tabla que lee: cualquier `authenticated` lee `payables` directo por PostgREST con su token. Por eso el rastro va en `audit_events` con RLS `is_admin()` y lo escribe un **trigger DEFINER**, no la ruta. `GET /api/payables` anexa `paid_by` **solo si el llamador es admin** — para un member la respuesta **no trae la llave** (ni `null`); `GET /api/payables/[id]/events` es `requireAdmin`; la columna "Pagada por" y el historial del popup solo existen con `useProfile().isAdmin` (`payablesColumns(isAdmin)`). La única prueba real es `tests/integration/payables-audit.integration.test.ts`. Ver ADR-028. |
