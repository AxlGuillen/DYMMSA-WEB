...
| 20260902043910 | add_finance_payables — módulo Finanzas fase 1 (issue #84): columna `payment_terms_days` en `suppliers` (días de crédito; NULL = contado, CHECK >= 0) y tabla `payables` (facturas por pagar: supplier FK **sin cascade** — borrar proveedor con facturas se bloquea —, concept/amount/invoice_date/due_date, status pending·paid·cancelled, `paid_at` fecha real de pago). Índice (status, due_date), trigger moddatetime, RLS + policy authenticated. Registro simbólico de egresos; la facturación oficial sigue en Odoo. |
| 20260909031135 | add_profiles_and_time_entries — módulo de horas (issue #93, ADR-026): `profiles` (1:1 con `auth.users`; `role` admin/member, `display_name`, `clock_employee_id` UNIQUE; trigger `handle_new_user` SECURITY DEFINER sobre `auth.users` + backfill), `is_admin()` (DEFINER, search_path fijo), `time_entries` (una fila por pareja de checada; `source_clock_in` inmutable como llave UNIQUE de idempotencia; rastro `edited_by/edited_at/original`; FK a profiles **sin cascade**), `time_imports` (bitácora) y RPC `import_time_entries` (transaccional, INVOKER, respeta filas editadas). **Primera RLS por persona** del esquema: SELECT propio o admin, escrituras solo admin. GRANTs explícitos sin `anon`. Paso de datos solo en la nube: roles admin de Axl y Diego + ids de checador 1/2/3/5. |
| 20260909162418 | import_time_entries_deterministic_dedupe — `CREATE OR REPLACE` de la RPC: el `DISTINCT ON` de la CTE `incoming` gana `ORDER BY user_id, work_date, clock_in, clock_out DESC NULLS LAST`. Sin él, una pareja repetida en el reporte con y sin salida conservaba una fila arbitraria (review PR #96). |
| 20260916190052 | tighten_profiles_and_imports_rls — review PR #99: `profiles` SELECT pasa de `USING (true)` a `id = auth.uid() OR is_admin()` y `time_imports` SELECT a `is_admin()`. El MCP lee con el token del usuario, así que la policy abierta era una puerta lateral a lo que `requireAdmin` cierra en la API. Además `CREATE OR REPLACE` de `import_time_entries`: el conteo `v_total` deduplica sobre los valores casteados, igual que la CTE `incoming`, para que `skipped_edited` no dependa de que el llamador normalice las horas. |
| 20260924031216 | add_audit_events — bitácora genérica (issue #100, ADR-028): tabla `audit_events` (`entity_type`, `entity_id`, `action`, `actor_id` (uuid informativo, sin FK), `actor_name` snapshot, `data` jsonb, `created_at`; índice por entidad). RLS: **solo el admin lee** (`is_admin()`), sin policy de escritura para `authenticated` y GRANT solo SELECT (sin `anon`). Función `audit_payable()` SECURITY DEFINER (search_path fijo, REVOKE de PUBLIC) + trigger `payables_audit` AFTER INSERT/UPDATE/DELETE: registra `created`, `status_changed`/`paid_at_changed` (from/to) y `deleted` con el nombre del actor. Smoke en la nube (rollback): un member marca pagada → el evento existe con su nombre; el member lee 0 filas y no puede insertar ni llamar la función; el admin lo ve. |
| 20260924034509 | audit_events_actor_without_fk — review PR #106: se quita el FK `actor_id → profiles`. Dentro del trigger, un JWT sin fila en `profiles` violaba el FK y tumbaba la escritura del usuario (23503 → 500); una bitácora nunca debe fallar la operación que registra. `actor_name` ya es el snapshot durable. |
| 20260924055249 | add_profile_shift — jornada por persona (issue #101, ADR-029): columna `shift` en `profiles` (`full_time` · `part_time` · NULL = sin asignar) para las líneas de referencia de las gráficas de horas. Paso de datos solo en la nube: todos `full_time` salvo Tania `part_time`. RLS sin cambios. |
