...
| (pendiente: aplicar en la nube) | import_time_entries_deterministic_dedupe — `CREATE OR REPLACE` de la RPC: el `DISTINCT ON` de la CTE `incoming` gana `ORDER BY user_id, work_date, clock_in, clock_out DESC NULLS LAST`. Sin él, una pareja repetida en el reporte con y sin salida conservaba una fila arbitraria (review PR #96). |
