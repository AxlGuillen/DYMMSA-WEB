...
| **Permisos por rol (#93)** | `is_admin()` + RLS en la BD, **`requireAdmin()`** en el handler (403 descriptivo) y `useProfile().isAdmin` en el cliente — las tres, nunca solo ocultando en el frontend. El `GET /api/time-entries` de un member **ignora `?user=`** y fuerza su propio id. Las correcciones de un admin nunca tocan `source_clock_in`. Ver ADR-026. |
