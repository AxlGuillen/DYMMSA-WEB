...
| **RLS** | Row Level Security de Supabase. Todas las tablas tienen RLS habilitado. Las tablas de datos requieren usuario autenticado; la aprobación por token usa `approval_token` como semáforo público. Desde #93, `time_entries` y `profiles` distinguen **por persona** (`auth.uid()` + `is_admin()`). | [[02-Arquitectura/Base-de-Datos]] |
| **role** (`profiles.role`) | `admin` ve y corrige las checadas de todos, importa el reporte y edita perfiles; `member` solo ve las suyas. Se aplica en BD (`is_admin()`), en el handler (`requireAdmin()`) y en la UI (`useProfile().isAdmin`). | [[04-Decisiones-Tecnicas/ADR-026-Perfiles-y-Permisos-por-Rol]] |
| **checada** | Pareja entrada/salida de un día (`time_entries`). `source = import` viene del reporte NGTeco; `manual`, de un admin. `source_clock_in` es inmutable y sirve de llave para que re-subir la semana no duplique. | [[04-Decisiones-Tecnicas/ADR-026-Perfiles-y-Permisos-por-Rol]] |
